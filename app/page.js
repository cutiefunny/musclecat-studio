...
// --- 데이터 Import ---
import { projects as allProjects } from '@/data/projects'; // 포트폴리오 슬라이더용
import { goodsItems as allGoodsItems } from '@/data/goods'; // 굿즈 데이터 가져오기
// ⬇️ (추가) 새 엔터테인먼트 데이터 임포트
import { entertainmentItems as allEntertainmentItems } from '@/data/entertainment'; 
// --- 데이터 Import End ---
...
export default function Home() {
  ...
  const [mobileGoodsIndex, setMobileGoodsIndex] = useState(0); 
  ...
  useEffect(() => {
    // --- 포트폴리오 이미지 셔플 ---
    // ⬇️ (수정) allProjects에서 portfolioImageUrls 추출
    const portfolioImageUrls = allProjects.map(p => p.imageUrl);
    const shuffledPortfolios = [...portfolioImageUrls].sort(() => Math.random() - 0.5); 
    setShuffledPortfolioImages(shuffledPortfolios);

    // --- 최신 굿즈 설정 로직 (랜덤 3개 선택) ---
    const shuffledGoods = [...allGoodsItems].sort(() => Math.random() - 0.5);
    setDisplayedGoods(shuffledGoods.slice(0, 3));
    
    // --- ⬇️ UPDATED: 엔터테인먼트 데이터 설정 (랜덤 3개 선택으로 수정) ---
    // data/entertainment.js에서 가져온 데이터를 섞고 상위 3개 선택
    const shuffledEntertainment = [...allEntertainmentItems].sort(() => Math.random() - 0.5);
    setDisplayedEntertainment(shuffledEntertainment.slice(0, 3));
    // --- 엔터테인먼트 데이터 설정 끝 ---
  ...
  // --- 모바일 엔터테인먼트 순환 로직 (유지) ---
  useEffect(() => {
    if (displayedEntertainment.length > 0) {
      const intervalId = setInterval(() => {
        setMobileEntertainmentIndex((prevIndex) => (prevIndex + 1) % displayedEntertainment.length);
      }, 5000); 
      return () => clearInterval(intervalId);
    }
    ...
    <div className={styles.page}>
      ...
      <div className={styles.buttonContainer}>
        ...
        <Link href="/entertainment"> 
          <Image src="/images/enter-button.png" alt="엔터테인먼트 보러가기" width={200} height={80} style={{ width: 'auto', height: '50px' }} />
        </Link>

        {/* (수정 없음) displayedEntertainment 배열 자체가 최대 3개이므로 렌더링 로직은 동일 */}
        <div className={styles.latestGoodsContainer}>
          {displayedEntertainment.length > 0 ? displayedEntertainment.map((item, index) => (
            <Link
              href={item.link}
              key={item.id}
              className={`${styles.goodsItemLink} ${styles.desktopGoodsItem} ${index === mobileEntertainmentIndex ? styles.mobileVisibleGoodsItem : styles.mobileHiddenGoodsItem}`}
              target="_blank"
            ...
            >
              <div className={styles.goodsItem}>
                ...
                <div className={styles.goodsContent}>
                  <h4 className={styles.goodsName}>{item.name}</h4>
                  {/* <p className={styles.goodsPrice}>{item.description}</p> */}
                </div>
              </div>
            </Link>
          )) : <p className={styles.noGoods}>엔터테인먼트가 없습니다.</p>}
        </div>
        {/* --- 엔터테인먼트 섹션 끝 --- */}
        ...
        {/* 최신 굿즈 표시 섹션 */}
        {/* (수정 없음) displayedGoods 배열 자체가 최대 3개이므로 렌더링 로직은 동일 */}
        <div className={styles.latestGoodsContainer}>
